...
- **BA-agent** (`ba-agent.md`). Business Analyst. Invoked by PM-agent (or directly by HAL) when a request requires research, data analysis, or structured decision-support information. Produces human-digestible summaries, evidence tables, pro/con matrices, and visualisation recommendations — always with source references. Flags to HAL when a topic exceeds its general expertise and a more specialised agent would improve quality. Uses memory to track investigated topics by domain cluster; HAL reviews these clusters weekly (interval set in `settings.json` → `baAgentReviewIntervalDays`) to decide whether a new specialist agent is warranted.
